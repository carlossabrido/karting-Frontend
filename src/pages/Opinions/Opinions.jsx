...
export const Opinions = () => {
  ...
  const updatesReviews = async () => {
    getOpinion()
      .then((resultado) => setReview(resultado))
      .catch((error) => console.log(error));
  };

  const makeReview = async () => {
    createReview(dataRdx.credentials, opinion).then((resultado) => {
      resultado;
    });
    updatesReviews();
    handleClose().catch((error) => console.log(error));
  };

  const removeReview = (opinionID) => {
    deleteReview(dataRdx.credentials, opinionID).then((resultado) => resultado);
    updatesReviews().catch((error) => console.log(error));
  };
          ...
          <div className="col-md-3 m-3" key={review.id}>
            <Card border="success" style={{ width: "18rem" }}>
              ...
              <Card.Body>
                ...
                {errors.opinion && (
                  <div className="error">{errors.opinion}</div>
                )}
                <Button
                  variant="danger"
